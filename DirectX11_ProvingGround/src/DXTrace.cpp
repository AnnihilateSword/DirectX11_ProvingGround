...
HRESULT __stdcall DXTraceW(const WCHAR* strFile, DWORD dwLine, HRESULT hr, const WCHAR* strMsg, bool bPopMsgBox)
{
	...
	// Windows SDK 8.0 起 DirectX 的错误信息已经集成进错误码中，可以通过 FormatMessageW 获取错误信息字符串
	// 不需要分配字符串内存
	...
	if (errorStr)
	{
		errorStr[0] = L'\0';  // 擦除 FormatMessageW 带来的换行符(把\r\n的\r置换为\0即可)
	}

	swprintf_s(strBufferHR, 40, L" (0x%0.8x)", hr);
	wcscat_s(strBufferError, strBufferHR);
	swprintf_s(strBuffer, 40, L"错误码含义：%ls", strBufferError);
	OutputDebugStringW(strBuffer);
